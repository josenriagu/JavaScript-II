...
runners.forEach(element => {
    fullName.push({ 'first_name': element.first_name, 'last_name': element.last_name });
});
...
// Problem 1 - return large sized shirt only if runner id is even
let largeShirtsEven = runners.filter((size) => {
    if (size.id % 2 === 0) {
        return size.shirt_size === 'L'
    }
});
